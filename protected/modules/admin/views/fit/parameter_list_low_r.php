        ...
        <?php if (!empty($data)): ?>
            <?php foreach ($data as $key => $value): ?>
                ...
                <tr class="<?php echo $className; ?>">
                    ...
                    <td><?php echo CHtml::link(CHtml::encode($value['name']), array('fit/parameterDetail', 'cid' => $value['cid']), array('target' => '_blank')); ?></td>
                    ...
                    <td>
                        <?php
                            $imgUrl = Yii::app()->baseUrl . "/images/figure/{$value["date"]}/{$value['cid']}_head.png";
                            echo CHtml::link(CHtml::image($imgUrl, '', array('width' => '250px', 'height' => '175px')), $imgUrl, array('target' => '_blank'));
                        ?>
                    </td>
                    <td style="text-align: right"><?php echo number_format($value['head_err'], 2); ?></td>
                    <td style="text-align: right"><?php echo number_format($value['head_r2'], 2); ?></td>
                    <td style="text-align: right"><?php echo number_format($value['tail_samples']); ?></td>
                    <td>
                        <?php
                            if ($value['tail_err'] || $value['tail_r2']) {
                                $imgUrl = Yii::app()->baseUrl . "/images/figure/{$value["date"]}/{$value['cid']}_tail.png";
                                echo CHtml::link(CHtml::image($imgUrl, '', array('width' => '250px', 'height' => '175px')), $imgUrl, array('target' => '_blank'));
                            } else {
                                echo '无';
                            }
                    ...
                    <td style="text-align: right"><?php echo number_format($value['tail_err'], 2); ?></td>
                    <td style="text-align: right"><?php echo number_format($value['tail_r2'], 2); ?></td>
                </tr>
            <?php endforeach; ?>
        <?php else: ?>
